...
apiHandler.configureApi = () => (
  new Promise((resolve, reject) => {
    ...
    if (!localStorage.getItem(apiKey) || !sessionStorage.getItem(apiKey)) {
      fetch(configUrl)
        .then((data) => data.json())
        .then((configData) => {
          localStorage.setItem(apiKey, JSON.stringify(configData));
          sessionStorage.setItem(apiKey, true);
        })
        .catch((e) => reject(e));
    }
...
apiHandler.locatePoster = (filmId, configObj) => (
  new Promise((resolve, reject) => {
    ...
    if (!localStorage.getItem(filmId) || !sessionStorage.getItem(filmId)) {
      fetch(`${BASE_URL}movie/${encodeURIComponent(filmId)}?api_key=${encodeURIComponent(apiKey)}&language=en-US`)
        ...
        .then((movie) => {
          if (movie.poster_path) {
            ...
            sessionStorage.setItem(filmId, true);
          } else {
